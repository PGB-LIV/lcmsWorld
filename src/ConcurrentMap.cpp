...
int concurrent_start()
{
	...
	db = new simdb("lcmsWorld", maxSize, 1024);

	if (whichProcess < 0)
	{
		auto val = db->get("Processes");
		if (val.length() == 0)
		{
			val = "0";
		...
		whichProcess = std::stoi(val);


		db->put("Processes", std::to_string(whichProcess + 1));
	}
	return 0;
}
...
void takeControl()
{
	...
	db->put("inControl", std::to_string(whichProcess));
...
bool processInControl()
{
	...
	auto val = db->get("inControl");
// 	std::cout << val << "  " << whichProcess << "\n";
	if (val == "-1")
	{
		takeControl();
		return true;
	}
	if (val == std::to_string(whichProcess))
		return true;
	return false;
}
