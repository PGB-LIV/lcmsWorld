...
MSSpectrumType RAWReader::evaluateFilter(long scan, char* chFilter, std::vector<double>& MZs, bool& bCentroid, double& cv, MSActivation& act) {
	...
	while (tok != NULL) {
		...
		if (strcmp(tok, "c") == 0) {
			...
			if (mzVal.compare("cid") == 0) {
				...
				std::cout << "Unknown activation method: " << &mzVal[0] << std::endl;
				act = mstNA;
			}
			...
			if (FAILED(m_Raw3.CreateInstance("MSFileReader.XRawfile.1"))) {
				if (FAILED(m_Raw2.CreateInstance("MSFileReader.XRawfile.1"))) {
					if (FAILED(m_Raw.CreateInstance("MSFileReader.XRawfile.1"))) {
						raw = 0;
						std::cout << "Cannot load Thermo MSFileReader. Cannot read .RAW files." << std::endl;
					}
					else {
						raw = 1;
					}
	...
	if (c == NULL) {
		...
		if (bNewFile) {
			...
			if (lRet != ERROR_SUCCESS) {
				std::cerr << "Cannot open " << c << std::endl;
				return false;
			}
...
void RAWReader::setMSLevelFilter(std::vector<MSSpectrumType>* v) {
	msLevelFilter = v;
}
