...
public:
    ...
    auto        alloc(u32    size, u32 klen, u32 hash, BlkCnt* out_blocks = nullptr) -> VerIdx
    {
        ...
        u32  nxt = 0;
        u32  cnt = 0;
        SECTION(loop for the number of blocks neededand get new blockand link it to the list)
        {
...
public:
    ...
    auto         get(str const& key) -> std::vector<T>
    {
        ...
        if (vlen == 0)
        {
            std::vector<T> ret_null;
            ret_null.clear();
            return ret_null;
        }
        std::vector<T> ret(vlen);
        get(key.data(), (u32)key.length(), (void*)ret.data(), vlen);
        return ret;
    }
    template<class T>
